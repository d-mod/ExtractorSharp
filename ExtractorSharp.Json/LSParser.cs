...
namespace ExtractorSharp.Json {
    ...
    public sealed class LSParser {
        ...
        private LSToken LookAhead() =>token = token != LSToken.None ? token : NextToken();


        private void Consume() => token = LSToken.None;
        ...
        private LSToken NextToken() {
            ...
            do {
                c = source[index];
                if (c == '/' && source[index + 1] == '/') {// c++ style single line comments
                    index += 2;
                    do {
                        c = source[index];
                        if (c == '\r' || c == '\n')
        ...
        private static bool IsIdentifier(char c) => char.IsLetter(c) || c == '_';
        ...
        private bool IsKeyword(string keyword) {
            if (source.Length - index >= keyword.Length) {
                if (source.Substring(index, keyword.Length).EqualsIgnoreCase(keyword)) {
                    if (source.Length - index >= keyword.Length) {
                        char c = source[index + keyword.Length];
