...
namespace System.Drawing {
    public static class Bitmaps {
        ...
        public static byte[] ToArray(this Bitmap bmp, ColorFormats type) {
            var data = bmp.ToArray();
        ...
        public static System.Windows.Media.ImageSource ToImageSource(this Bitmap bitmap) {
            ...
            return Windows.Media.Imaging.BitmapSource.Create(width, height, 0, 0, System.Windows.Media.PixelFormats.Bgra32, null, data, 4 * width);
        }
        ...
        /// <returns></returns>
        public static Bitmap FromArray(byte[] data, Size size, ColorFormats bits) {
            data = ConvertTo32Bits(data, bits);
            return FromArray(data, size);
        }


        public static byte[] ConvertTo32Bits(byte[] data, ColorFormats bits) {
        ...
        public static byte[] ConvertToBits(byte[] data, ColorFormats bits) {
            if(bits != ColorFormats.ARGB_8888) {
                var ms = new MemoryStream();
                for(var i = 0; i < data.Length; i += 4) {
                    var buf = new byte[4];
                    Array.ConstrainedCopy(data, i, buf, 0, 4);
                    Colors.WriteColor(ms, buf, bits);
                }
                ms.Close();
                data = ms.ToArray();
            }
