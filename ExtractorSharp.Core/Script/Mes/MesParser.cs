...
namespace ExtractorSharp.Script.Mes {
    ...
    public struct TokenInvokeParameter {
        ...
        public Token LastToken => this.Tokens[this.Cursor - 1];
        public Token CurrentToken => this.Tokens[this.Cursor];
        public Token NextToken => this.Tokens[this.Cursor + 1];
    }
    ...
    public class MesParser {
        ...
        public MesParser() {
            this.FuncMap = new Dictionary<string, Func<TokenInvokeParameter, TokenInvokeResult>> {
                ...
                ["useVar"] = arg => {
                    var value = this.CalculatePropertyValue(arg.NextToken.Text);
                ...
                ["forEach"] = arg => {
                    ...
                    foreach(var i in iterationVariable) {
                        ...
                        // Console.WriteLine(i);
                        this.InvokeToken(block.ContentTokens);
                    }
                    ...
                    foreach(var t in block.ContentTokens.Skip(2)) // 越过API名及其分号
                    {
                        switch(t) {
                            case LineEndToken token:
                                ...
                                APIPars.Add(this.InvokeToken(tokens));
                                tokens.Clear();
                                break;
                            case Token token:
                                tokens.Add(token);
                ...
                ["Concat"] = arg => {
                    ...
                    var nextTokenValue = this.InvokeToken(arg.Tokens.Skip(arg.Cursor + 1).ToList(), out var nextTokenResult, true);
        ...
        public bool CheckSwitch(string name) {
            return this._status.ContainsKey(name) && this._status[name] as string == "on";
        }
        /// <summary>
        ///     用于设置某个开关状态
        /// </summary>
        public bool SetSwitch(string name, bool status) {
            this._status[name] = status ? "on" : "off";
            return status;
        }
        /// <summary>
        ///     用于设置某个开关的值
        ...
        public object GetSwitch(string name, object defaultValue = null) {
            return this._status.TryGetValue(name, out var value) ? value : defaultValue;
        }
        /// <summary>
        ...
        public object InvokeToken(List<Token> tokens, out TokenInvokeResult outResult, bool stopWhenLineEnd = false) {
            ...
            for(var i = 0; i < tokens.Count; i++) {
                ...
                if(this.FuncMap.TryGetValue(parameter.CurrentToken.Text, out var fun)) {
        ...
        public object InvokeToken(List<Token> tokens, bool stopWhenLineEnd = false) {
            TokenInvokeResult _;
            return this.InvokeToken(tokens, out _, stopWhenLineEnd);
        }
        public object InvokeToken(string tokens, bool stopWhenLineEnd = false) {
            return this.InvokeToken(this.ParseBlock(tokens), stopWhenLineEnd);
        }
                ...
                switch(theChar) {
                    ...
                    case '}':
                        ...
                        if(leftBraceCount == 0) {
                            ...
                            currentBlock.ContentTokens = this.ParseBlock(currentBlock.Text);
            ...
            foreach(var t in tokens) {
                ...
                switch(t) {
                    case LineEndToken _t:
                        ...
                        goto default;
                    case BlockToken _t:
                        currentLine.Append(this.GetAST(_t.ContentTokens, spaceCount + 4));
                        break;
                    case Token _t:
                        currentLine.Append(_t.Text);
                        goto default;
