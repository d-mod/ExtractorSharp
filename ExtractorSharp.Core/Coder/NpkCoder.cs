    ...
    public static class NpkCoder {
        ...
        private static byte[] Key {
            get {
                ...
                var cs = new byte[256];
                var length = Encoding.GetBytes(KEY_HEADER, 0, KEY_HEADER.Length, cs, 0);
        ...
        /// <returns></returns>
        public static Bitmap ReadImage(Stream stream, Sprite sprite) {
            var data = new byte[sprite.Width * sprite.Height * 4];
            for(var i = 0; i < data.Length; i += 4) {
                var bits = sprite.ColorFormat;
                if(sprite.Version == ImgVersion.Ver4 && bits == ColorFormats.ARGB_1555) {
                    bits = ColorFormats.ARGB_8888;
                }
                Colors.ReadColor(stream, bits, data, i);
            }
            return Bitmaps.FromArray(data, sprite.Size);
        }
        ...
        public static void Compare(string gamePath, Action<Album, Album> restore, IEnumerable<Album> array) {
        ...
        public static void Compare(string gamePath, string dir, Action<Album, Album> restore, IEnumerable<Album> array) {
