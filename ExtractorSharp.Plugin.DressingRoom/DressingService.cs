...
namespace ExtractorSharp.Service {
    public class DressingService {
        ...
        private readonly Dictionary<string, WeaponInfo> weapon_info;
        private readonly Dictionary<string, string> replace_dic;
        ...
        private  string RESOUCES_URL => Config["ResourceUrl"].Value;
        private  string API_URL => Config["ApiUrl"].Value;
        ...
        private IEnumerable<Album> FindImg(string profession, string part, int code) {
            ...
            string path = $"{ Config["ResourcePath"]}/sprite_character_{ profession }equipment_avatar_{ name }.NPK";
            if (!File.Exists(path)) {
                return new List<Album>();
            }
        ...
        private IEnumerable<Album> FindWeapon(string prefession, string type, int code) {
            ...
            var path = $"{ Config["ResourcePath"]}/sprite_character_{ prefession }equipment_weapon_{ type }.NPK";
            if (!File.Exists(path)) {
                return new List<Album>();
            }
